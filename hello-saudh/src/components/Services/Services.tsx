          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-16">
            {services.map((service, index) => (
              <Link key={service.title} to={service.link} className="block no-underline">
                <motion.div
                  className="p-8 border border-gray-200 rounded-2xl hover:shadow-2xl transition-all duration-300 hover:border-[#ffd00a] group"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6, delay: index * 0.1 }}
                  whileHover={{ scale: 1.02 }}
                >
                  <h3 className="text-3xl font-bold text-gray-900 mb-4 group-hover:text-[#ffd00a] transition-colors">{service.title}</h3>
